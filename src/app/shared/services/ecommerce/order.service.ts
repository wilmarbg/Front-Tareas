...
import { orderHistoryTable } from '../../data/ecommerce/orderhistory';
...
function matches(details: ORDERS, term: string) {
...
export class OrderService {
  ...
  private _search(): Observable<SearchResult> {
    ...
    // 1. sort
    let support = sort(orderHistoryTable, sortColumn, sortDirection);

    // 2. filter
    support = support.filter(ticket => matches(ticket, searchTerm));
